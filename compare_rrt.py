        ...
        for _ in range(MAX_ITER):
            ...
            if node_new and not is_collision(node_near, node_new, OBS_RECTANGLE, OBS_CIRCLE, ROBOT_RADIUS):
                ...
                for i in range(len(self.s_goal)):
                    ...
                    if dist <= STEP_LENGTH and not is_collision(node_new, self.s_goal[i], OBS_RECTANGLE, OBS_CIRCLE, ROBOT_RADIUS):
...
if __name__ == "__main__":
    ...
    if success:
        ...
        with open('data_rrt/scen{}_rrt_{}.txt'.format(scenario, counter), 'wb') as f:
            d = dict()
            d["paths"] = paths
            d["pt"] = pt
            pickle.dump(d, f)
